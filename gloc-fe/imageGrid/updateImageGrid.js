...
import {animationType, updateOnlyDifferentImg} from "../uiElements/sidePanel.js"; // Replace with the actual path to your constant
import {clearRecognitionIntervals, getRecognitionInterval} from '../faceRecognition.js'; // Adjust the path as needed
...
import {createTextOverlay} from "./createImageContainer.js";
import {} from "./updateShuffle.js";
...
async function updateImages(images, imageData, abortController) {
    ...
    for (let i = 0; i < numImages; i++) {
        ...
        const newTextOverlay = createTextOverlay(i, imageData);
        ...
        if (shouldUpdateImage(images[i], currentImage)) {
            if (animationType === "none") {
                ...
                textOverlay.innerHTML = newTextOverlay.innerHTML;
            } else {
                if (animationType === "crossfade") {
                    const crossfadePromise = updateCrossfade(currentImage, nextImage, textOverlay, newTextOverlay, fadeOverlay, images[i].src);
                    animationPromises.push(crossfadePromise);
                } else if (animationType === "fullFade") {
                    const fadePromise = updateFullFade(currentImage, nextImage, textOverlay, newTextOverlay, fadeOverlay, images[i].src);
...
async function updateCrossfade(currentImage, nextImage, textOverlay, newTextOverlay, fadeOverlay, imageSrc) {
    return new Promise((resolve) => {
        currentImage.style.transition = `opacity ${fadeTime/2}s linear`;
        nextImage.style.transition = `opacity ${fadeTime/2}s linear`;
        textOverlay.style.transition = `opacity ${fadeTime/2 / 2}s linear`;
...
async function updateFullFade(currentImage, nextImage, textOverlay, newTextOverlay, fadeOverlay, imageSrc) {
    return new Promise((resolve) => {
        currentImage.style.transition = `opacity ${fadeTime/2}s linear`;
        textOverlay.style.transition = `opacity ${fadeTime/2 / 2}s linear`;
        fadeOverlay.style.transition = `opacity ${fadeTime/2}s linear`;
    ...
    export function initFadeSlider() {
        ...
        timeSlider.addEventListener("input", function () {
            sliderValue.textContent = timeSlider.value + 's';
            fadeTime = parseInt(timeSlider.value); // Convert to integer and update updateCount
            clearRecognitionIntervals()
        });
    }